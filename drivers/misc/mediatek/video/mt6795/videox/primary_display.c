...
DISP_MODE get_primary_session_mode(void)
{
...
#ifdef CONFIG_FOR_SOURCE_PQ
static void _primary_mira_path_lock(void)
{
	disp_sw_mutex_lock(&(pgc->mira_lock));
}

static void _primary_mira_path_unlock(void)
{
	disp_sw_mutex_unlock(&(pgc->mira_lock));
}

int primary_display_mira_path_connected(int need_lock) {
    int ret = 0;
    if (need_lock)
        _primary_mira_path_lock();

	ret = (pgc->dpmgr_sub_mem_path_handle != NULL) ? 1 : 0;

    if (need_lock)
        _primary_mira_path_unlock();

    return ret;
}

static void primary_display_suspend_miravision (void)
{
    _primary_mira_path_lock();
    if (primary_display_mira_path_connected(0)) {
		DISPFUNC();
        dpmgr_path_stop(pgc->dpmgr_sub_mem_path_handle, CMDQ_DISABLE);
        dpmgr_path_reset(pgc->dpmgr_sub_mem_path_handle, CMDQ_DISABLE);
        dpmgr_path_power_off(pgc->dpmgr_sub_mem_path_handle, CMDQ_DISABLE);
    }
    _primary_mira_path_unlock();
}

static void primary_display_resume_miravision (void)
{
    _primary_mira_path_lock();
    if (primary_display_mira_path_connected(0)) {
		DISPFUNC();
        dpmgr_path_power_on(pgc->dpmgr_sub_mem_path_handle, CMDQ_DISABLE);
        dpmgr_path_start(pgc->dpmgr_sub_mem_path_handle, CMDQ_DISABLE);
        dpmgr_path_reset(pgc->dpmgr_sub_mem_path_handle, CMDQ_DISABLE);
    }
    _primary_mira_path_unlock();
}

int primary_display_init_path_miravision(void)
{
	int ret = 0;
	DISPFUNC(); 

    _primary_mira_path_lock();
    if (pgc->dpmgr_sub_mem_path_handle == NULL) {
    	...
    	dpmgr_path_init(pgc->dpmgr_sub_mem_path_handle, CMDQ_DISABLE);
    	dpmgr_path_start(pgc->dpmgr_sub_mem_path_handle, CMDQ_DISABLE);

    	dpmgr_enable_event(pgc->dpmgr_sub_mem_path_handle, DISP_PATH_EVENT_FRAME_COMPLETE);
    	dpmgr_map_event_to_irq(pgc->dpmgr_sub_mem_path_handle, DISP_PATH_EVENT_FRAME_COMPLETE, DDP_IRQ_WDMA1_FRAME_COMPLETE);
    }
	_primary_mira_path_unlock();
...
int primary_display_deinit_path_miravision(void)
{
	int ret = 0;
	DISPFUNC(); 
    ...
    _primary_mira_path_lock();
    if (pgc->dpmgr_sub_mem_path_handle != NULL) {
        dpmgr_wait_event_timeout(pgc->dpmgr_sub_mem_path_handle, DISP_PATH_EVENT_FRAME_COMPLETE, HZ/100);
        dpmgr_path_stop(pgc->dpmgr_sub_mem_path_handle, CMDQ_DISABLE);
        dpmgr_path_reset(pgc->dpmgr_sub_mem_path_handle, CMDQ_DISABLE);
        
    	dpmgr_path_deinit(pgc->dpmgr_sub_mem_path_handle, CMDQ_DISABLE);
    	dpmgr_destroy_path(pgc->dpmgr_sub_mem_path_handle, NULL);
    	
    	pgc->dpmgr_sub_mem_path_handle = NULL;
    }
    _primary_mira_path_unlock();
...
#ifdef CONFIG_FOR_SOURCE_PQ
	/* power off miravision path if it's connected */
	primary_display_suspend_miravision();
#endif
...
extern int dis_lcm_power_resume(disp_lcm_handle *plcm);/* MM-GL-DISPLAY-panel-24+ */
int primary_display_resume(void)
{
	...
	dis_lcm_power_resume(pgc->plcm);/* MM-GL-DISPLAY-panel-24+ */
...
#ifdef CONFIG_FOR_SOURCE_PQ
	/* It's need to power on the mira path if it's connected */
	primary_display_resume_miravision();
#endif
...
static DISP_MODE session_mode_backup;
static int disp_enter_svp(SVP_STATE state)
{
	int i;

	if (state == SVP_IN_POINT) {
		session_mode_backup = get_primary_session_mode();
		if (!is_backup) {
...
static int disp_leave_svp(SVP_STATE state)
{
	...
	if (state == SVP_EXIT_POINT) {
		...
		if (session_mode_backup == DISP_SESSION_DECOUPLE_MODE)
			__primary_display_switch_path(session_mode_backup, pgc->session_id, 0, DISP_USER_SVP);
	}
	return 0;
}
...
int primary_display_user_cmd(unsigned int cmd, unsigned long arg)
{
	...
	int sub_size = 0;
    DISP_PQ_STATUS st = {0};
    //To confirm if end user enable/disable it
    // fixed: normal path PQ config is dispatched to wrong handle
    if(cmd == DISP_IOCTL_PQ_GET_DISP_STATUS) {
        if (copy_from_user(&st, (void __user *)arg, sizeof(DISP_PQ_STATUS))) {
	...
	if (_need_apply_on_pq_path(cmd)) {
	    _primary_mira_path_lock();
	    if (st.enable && primary_display_mira_path_connected(0)) {
