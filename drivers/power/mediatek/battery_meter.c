...
/* CORE-EL-dynamic_car_tune_value-00+ */
static kal_bool car_tune_stage = 0;
...
/* CORE-EL-MULTI_BAT_ID-00+[ */
enum EN_BATT_ID {
	BATT_ID_1 = 1,
	BATT_ID_2,
	BATT_ID_3,
	BATT_ID_4,
	BATT_ID_UNKNOWN = BATT_ID_4,
	BATT_ID_5,
}; 

struct ST_BATT_ID_INFO{
	enum EN_BATT_ID id;
	int maxv;
	int minv;
};

struct ST_BATT_ID_INFO st_batt_id_info[] = {
	{BATT_ID_1, 255, 116},
	{BATT_ID_2, 115, 101},
	{BATT_ID_3, 100, 87},
	{BATT_ID_4, 86, 73},
...
enum EN_BATT_ID g_battery_id = BATT_ID_UNKNOWN;

extern int IMM_GetOneChannelValue(int dwChannel, int data[4], int* rawdata);
void get_battery_id(void) {

	int idx;
	int bat_id_volt = 80;
	int data[4] = {0};

	/* get battery ID pin voltage */
	IMM_GetOneChannelValue(1, data, NULL);

	bat_id_volt = data[0] * 100 + data[1];
	bm_print(BM_LOG_CRTI, "bat_id_volt = %d\n", bat_id_volt);

	for(idx = 0; idx < sizeof(st_batt_id_info) / sizeof(st_batt_id_info[0]); idx ++) {
		if ((st_batt_id_info[idx].maxv >= bat_id_volt) &&
			 (st_batt_id_info[idx].minv <= bat_id_volt)) {
			 g_battery_id = st_batt_id_info[idx].id;
			 break;
		}
	}

	bm_print(BM_LOG_CRTI, "bat_id = %d\n", g_battery_id);
...
int get_car_tune_value(void) {
	static int car_tune_value = 100;
	static unsigned int phase_id = PHASE_AP2;

	if (likely(car_tune_stage == 2))
		goto exit;
	else{
		phase_id = fih_get_product_phase();

		/* SP5/AP2/TP/PQ/MP uses new car tune value */
		switch (phase_id) {
			case PHASE_EVM:
			case PHASE_EVM2:
			case PHASE_EVM3:
			...
			case PHASE_AP:
				car_tune_value = 
					(car_tune_stage == 0) ? CAR_TUNE_VALUE_OLD_DOD0: CAR_TUNE_VALUE_OLD;
				break;
			default:
				car_tune_value = 
					(car_tune_stage == 0) ? CAR_TUNE_VALUE_NEW_DOD0: CAR_TUNE_VALUE_NEW;
				break;
		}
		if (car_tune_stage == 1)
			car_tune_stage = 2;
	...
	bm_print(BM_LOG_CRTI, "phase:%x, car_tune_value:%d, %d\n", phase_id, car_tune_value, car_tune_stage);
exit:
	
	return car_tune_value;
}
/* CORE-EL-dynamic_car_tune_value-01*] */
...
// CORE-EL-MORE_ACCURATE_TEMP-00+[
int BattThermistorConverTempAccur(int Res)
{
	int i = 0;
	int RES1 = 0, RES2 = 0;
	int TBatt_Value = -200, TMP1 = 0, TMP2 = 0;

	if (Res >= Batt_Temperature_Table_Accur[0].TemperatureR) {
		TBatt_Value = -200;
	} else if (Res <= Batt_Temperature_Table_Accur[16].TemperatureR) {
		TBatt_Value = 600;
	} else {
		RES1 = Batt_Temperature_Table_Accur[0].TemperatureR;
		TMP1 = Batt_Temperature_Table_Accur[0].BatteryTemp;

		for (i = 0; i <= 16; i++) {
			if (Res >= Batt_Temperature_Table_Accur[i].TemperatureR) {
				RES2 = Batt_Temperature_Table_Accur[i].TemperatureR;
				TMP2 = Batt_Temperature_Table_Accur[i].BatteryTemp;
				break;
			} else {
				RES1 = Batt_Temperature_Table_Accur[i].TemperatureR;
				TMP1 = Batt_Temperature_Table_Accur[i].BatteryTemp;
			}
		}
...
int BattVoltToTempAccur(int dwVolt)
{
		kal_int64 TRes_temp;
		kal_int64 TRes;
		int sBaTTMP = -100;
		...
		sBaTTMP = BattThermistorConverTempAccur((int)TRes);
	
		return sBaTTMP;
}

int force_get_tbat_accur(kal_bool update)
{
#if defined(CONFIG_POWER_EXT) || defined(FIXED_TBAT_25)
	bm_print(BM_LOG_CRTI, "[force_get_tbat_accur] fixed TBAT_ACCUR=250 t\n");
	return 250;
#else
	...
	if (update == KAL_TRUE || pre_bat_temperature_val == -1) {
		...
		if (bat_temperature_volt != 0) {
			...
			bat_temperature_val = BattVoltToTempAccur(bat_temperature_volt);
		}

	bm_print(BM_LOG_CRTI, "[force_get_tbat_accur] %d,%d,%d,%d,%d,%d\n",
...
#endif
	{
		/* If "rtc_soc > cur_soc", let "cur_soc = rtc_soc - 1", to let cur_soc more 
		    closer to rtc_soc at each power cycle */
		if ((g_rtc_fg_soc != 100) && (g_rtc_fg_soc > gFG_capacity_by_v)) {
...
/* CORE-EL-show_bat_info_now-01*[ */
void get_bat_info_now(kal_int32 *pvolt, 
		kal_int32 *pcurrent, kal_int32 *ptemp) 
{
	*pvolt = g_fg_dbg_bat_volt;
	*pcurrent = g_fg_dbg_bat_current;
	*ptemp = force_get_tbat_accur(KAL_FALSE);
}
/* CORE-EL-show_bat_info_now-01*] */
						      ...
						      kal_int32 recursion_time)
{
	...
	if (car_tune_stage == 0) {
		if (times++ >= 7) {
			/* when DOD0 got, car_tune_stage changed to 1 */
				car_tune_stage = 1;
		}
	}
...
kal_int32 fgauge_update_dod(void)
{
	...
	int adjust_coulomb_counter = get_car_tune_value();
#ifdef Q_MAX_BY_CURRENT
	kal_int32 C_0mA = 0;
	kal_int32 C_400mA = 0;
...
/* 5. Logging */
	...
	bm_print(BM_LOG_CRTI,
		 ...
		 gFG_DOD1, get_car_tune_value(), AGING_TUNING_VALUE);
...
kal_int32 get_dynamic_period(int first_use, int first_wakeup_time, int battery_capacity_level)
{
	...
	*/
	...
	if (vbat_val > VBAT_NORMAL_WAKEUP)	{	/* 3.6v */
		...
		car_sleep = car_wakeup;
		last_time = ret_val;
		g_spm_timer = (ret_val > 3600)?ret_val:3600; //CORE-BH-ReducePCMWakeup-00*
	} else if (vbat_val > VBAT_LOW_POWER_WAKEUP) {	/* 3.5v */
		g_spm_timer = LOW_POWER_WAKEUP_PERIOD;	/* 5 min */
	} else {
		g_spm_timer = CLOSE_POWEROFF_WAKEUP_PERIOD;	/* 0.5 min */
...
/* ------------------------------------------------------------------------------------------- */
// CORE-EL-MORE_ACCURATE_TEMP-00+[
static ssize_t show_FG_g_fg_dbg_bat_temp_accur(struct device *dev, struct device_attribute *attr,
					 char *buf)
{
	int fg_dbg_bat_temp_accur = force_get_tbat_accur(KAL_FALSE);
	bm_print(BM_LOG_CRTI, "[FG] fg_dbg_bat_temp_accur : %d\n", fg_dbg_bat_temp_accur);
	return sprintf(buf, "%d\n", fg_dbg_bat_temp_accur);
}

static ssize_t store_FG_g_fg_dbg_bat_temp_accur(struct device *dev, struct device_attribute *attr,
					  const char *buf, size_t size)
{
	return size;
}

static DEVICE_ATTR(FG_g_fg_dbg_bat_temp_accur, 0664, show_FG_g_fg_dbg_bat_temp_accur,
		   store_FG_g_fg_dbg_bat_temp_accur);
/* ------------------------------------------------------------------------------------------- */
// CORE-EL-MORE_ACCURATE_TEMP-00+]
...
static int battery_meter_probe(struct platform_device *dev)
{
	...
	/* CORE-EL-dynamic_car_tune_value-00+ */
	/* get car tune value according to device phase */
	get_car_tune_value();
	...
	// CORE-EL-MORE_ACCURATE_TEMP-00+
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_FG_g_fg_dbg_bat_temp_accur);
